export const markdownitWidgetPlugin = (md, options: { name: string; withContent?: boolean }) => {
  const markdownitWidgetParser = (state, silent: boolean) => {
    ...
    const endParamsPos = state.src.indexOf(closingParamsTag, widgetStartPos + openingTag.length);
    ...
    const endContentPos = options.withContent ? state.src.indexOf(closingContentTag, widgetStartPos + openingTag.length) : endClosingParamsTagPos;

    const endClosingContentTagPos = endContentPos + closingContentTag.length;

    const paramsString = state.src.slice(widgetStartPos + openingTag.length, endParamsPos).trim();

    const content = options.withContent ? state.src.slice(endClosingParamsTagPos, endContentPos) : '';
    ...
    state.pos = options.withContent ? endClosingContentTagPos : endClosingParamsTagPos;

    return true;
  };

  md.inline.ruler.before('emphasis', `${options.name}-widget`, markdownitWidgetParser);
};
